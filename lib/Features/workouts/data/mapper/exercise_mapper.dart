...
import 'package:fitness_app/core/helpers/youtube_utils.dart';

extension ExerciseRemoteMapper on Exercise {
  ExerciseHiveModel toHiveModel() {
    return ExerciseHiveModel(
      id: id ?? '',
      title: [primeMoverMuscle, primaryEquipment]
    .where((e) => e != null && e.isNotEmpty)
    .join(' • '),
      description: exercise ?? 'Unknown Exercise',
      sets: 3,
      reps: 15,
    thumbnailUrl: (shortYoutubeDemonstrationLink ?? inDepthYoutubeExplanationLink)
    ?.getYoutubeThumbnail(quality: ThumbnailQuality.high) ?? '',
videoUrl: inDepthYoutubeExplanationLink ?? shortYoutubeDemonstrationLink,
    );
  }
}
